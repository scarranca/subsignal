...
export const dealQueries = {
    ...
    async createDeal(
        ...
        userId: string,
        data: Omit<DealInsert, 'id' | 'organizationId' | 'userId' | 'createdAt' | 'updatedAt'>
    ): Promise<DealSelect> {
        const id = nanoid();
        const now = new Date();
    ...
    async getDealByIdForOrg(dealId: string, organizationId: string): Promise<DealWithRelations> {
        const result = await db.query.deal.findFirst({
            where: and(eq(deal.id, dealId), eq(deal.organizationId, organizationId), eq(deal.isActive, true)),
            with: {
                company: {
    ...
    async getOrganizationDeals(
        ...
        const conditions = [eq(deal.organizationId, organizationId), eq(deal.isActive, true)];
        ...
        if (ownerId) conditions.push(eq(deal.ownerId, ownerId));
    ...
    async getPipelineDealsForOrg(organizationId: string, pipelineId: string): Promise<PipelineDeals> {
        ...
        const deals = await db.query.deal.findMany({
            where: and(eq(deal.organizationId, organizationId), eq(deal.pipelineId, pipelineId), eq(deal.isActive, true), eq(deal.status, 'open')),
            with: {
                company: {
                    columns: { id: true, name: true },
                },
    ...
    async updateDealForOrg(
        dealId: string,
        organizationId: string,
        data: Partial<Omit<DealInsert, 'id' | 'organizationId' | 'userId' | 'createdAt'>>
    ): Promise<DealSelect> {
    ...
    async moveDealToStageForOrg(dealId: string, organizationId: string, stageId: string): Promise<DealSelect> {
    ...
    async deleteDealForOrg(dealId: string, organizationId: string): Promise<{ success: boolean }> {
    ...
    async getDealStatsForOrg(organizationId: string, pipelineId?: string): Promise<{
        totalDeals: number;
        openDeals: number;
        wonDeals: number;
        lostDeals: number;
    ...
    async getDealCountForOrg(organizationId: string): Promise<number> {
        const result = await db
            .select({ count: sql<number>`count(*)::int` })
            .from(deal)
            .where(and(eq(deal.organizationId, organizationId), eq(deal.isActive, true)));

        return result[0]?.count || 0;
    },

    async assignOwner(dealId: string, organizationId: string, ownerId: string): Promise<DealSelect> {
    ...
    async getDealsByOwner(organizationId: string, ownerId: string): Promise<DealSelect[]> {
        return db.query.deal.findMany({
            where: and(
                eq(deal.organizationId, organizationId),
                eq(deal.ownerId, ownerId),
                eq(deal.isActive, true)
            ),
        });
    },
